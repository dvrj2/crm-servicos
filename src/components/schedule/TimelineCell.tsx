...
export function TimelineCell({ tech, date, orders, onDropOS }: Props) {
  ...
  const totalTravel = sorted.reduce((sum, os) => sum + (os.estimated_travel_hours || 0), 0)
  const capacity = tech.capacity_diaria_hours || 8
  const occPct = capacity > 0 ? (totalDuration / capacity) * 100 : 0
  const travelPct = capacity > 0 ? (totalTravel / capacity) * 100 : 0
